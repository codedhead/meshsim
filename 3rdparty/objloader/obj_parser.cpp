...
{
	// newly added
	memset(mtl,0,sizeof(obj_material));
	mtl->transmit_filter[0] = 1.0;
	mtl->transmit_filter[1] = 1.0;
	mtl->transmit_filter[2] = 1.0;
...
obj_vector* obj_parse_vector()
{
	...
	char* n3 = strtok(NULL, WHITESPACE); // in case of two tex coords
	v->e[2] = n3 ? atof( n3 ) : 0.;
	return v;
}
...
int obj_parse_mtl_file(char *filename, list *material_list)
{
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		//transimission filter
		else if( strequal(current_token, "Tf") && material_open)
		{
			current_mtl->transmit_filter[0] = atof( strtok(NULL, " \t"));
			current_mtl->transmit_filter[1] = atof( strtok(NULL, " \t"));
			current_mtl->transmit_filter[2] = atof( strtok(NULL, " \t"));
		}
		...
		//transparent
		else if( (strequal(current_token, "d") || strequal(current_token, "Tr")) && material_open)
		{
		...
		{
			strncpy(current_mtl->map_Kd, strtok(NULL, WHITESPACE), OBJ_FILENAME_LENGTH);
		}
		// specular map
		else if( strequal(current_token, "map_Ks") && material_open)
		{
			strncpy(current_mtl->map_Ks, strtok(NULL, WHITESPACE), OBJ_FILENAME_LENGTH);
		}
		// alpha mask
		else if( strequal(current_token, "map_d") && material_open)
		{
			strncpy(current_mtl->map_d, strtok(NULL, WHITESPACE), OBJ_FILENAME_LENGTH);
		}
		// bump
		else if( (strequal(current_token, "bump") || strequal(current_token, "map_bump")) && material_open)
		{
			strncpy(current_mtl->bump, strtok(NULL, WHITESPACE), OBJ_FILENAME_LENGTH);
		}
		else
		{
...
obj_group* get_last_group(obj_growable_scene_data *growable_data)
{
	obj_group* last_group=NULL;
	if(growable_data->group_list.item_count>0)
		last_group=(obj_group*)list_get_index(&growable_data->group_list,growable_data->group_list.item_count-1);
	return last_group;
}

#include <assert.h>

bool should_start_new_group(obj_growable_scene_data *growable_data)
{
	obj_group* last_group=get_last_group(growable_data);

	if(last_group)
	{
		assert(last_group->face_index_begin<=growable_data->face_list.item_count);
		
		return (last_group->face_index_begin<growable_data->face_list.item_count);
	}
	else
	{
		assert(growable_data->face_list.item_count==0); // name before data
...
/*		
	...
	while( true )
	{
		// get until line ends, assume that line content would not exceed OBJ_LINE_SIZE
		char* continue_get=current_line;bool _got=false;
		while(fgets(continue_get, OBJ_LINE_SIZE, obj_file_stream))
		{
			line_number++;

			int len=strlen(continue_get);
			if(len==0) break;
			_got=true;
			char* p=continue_get+len-1;
			while(p>=continue_get&&isspace(*p))--p;
			if(p>=continue_get&&*p=='\\') // continue get line
			{
				continue_get=p;
			}
			else
		...
		else if( strequal(current_token, "f") ) //process face
		{
			...
			// [add]
			if(face->vertex_count==4) // split counter-clockwise
			{
				obj_face *face2 = (obj_face*)malloc(sizeof(obj_face));
				face2->vertex_count = face->vertex_count = 3;
				face2->material_index = face->material_index;

#define __COPY_IDX(dst,src) dst[0]=src[2];dst[1]=src[3];dst[2]=src[0];/*dst[3]=-1;*/	

				__COPY_IDX(face2->vertex_index,face->vertex_index);
				__COPY_IDX(face2->normal_index,face->normal_index);
				__COPY_IDX(face2->texture_index,face->texture_index);

				list_add_item(&growable_data->face_list, face2, NULL);
			}
		}
		...
		{
			// static bool name_before_group=false;
			// assert group name before actual faces

			if(should_start_new_group(growable_data))
			{
				obj_group *newgroup = (obj_group*)malloc(sizeof(obj_group));
				newgroup->face_index_begin = growable_data->face_list.item_count;
				newgroup->material_index = current_material;
				// get the name
				char* gname=strtok(NULL, WHITESPACE);
				if(gname)
					strncpy(newgroup->name, gname, MATERIAL_NAME_SIZE);
				else
					strncpy(newgroup->name, "anonymousg", MATERIAL_NAME_SIZE);

				list_add_item(&growable_data->group_list, newgroup, newgroup->name);
			}
			else
			{
				obj_group *last_group=get_last_group(growable_data);
				assert(last_group);
				// update the name
				strncpy(last_group->name, strtok(NULL, WHITESPACE), MATERIAL_NAME_SIZE);
			}			
		}	
		else if( strequal(current_token, "usemtl") ) // usemtl
		{
			char* mat_name=strtok(NULL, WHITESPACE);
			current_material = list_find(&growable_data->material_list, mat_name);
			...
			if(should_start_new_group(growable_data))
			{
				...
				newgroup->material_index = current_material;
				// get the name
				sprintf(newgroup->name, "%s_group", mat_name);
			...
			else
			{
				...
				last_group->material_index = current_material;
			}
		}
...
void obj_init_temp_storage(obj_growable_scene_data *growable_data)
{
	...
	list_make(&growable_data->group_list, 10, 1);
...
void obj_free_temp_storage(obj_growable_scene_data *growable_data)
{
	...
	obj_free_half_list(&growable_data->group_list);
...
void delete_obj_data(obj_scene_data *data_out)
{
	...
	for(i=0; i<data_out->group_count; i++)
		free(data_out->group_list[i]);
	free(data_out->group_list);
...
void obj_copy_to_out_storage(obj_scene_data *data_out, obj_growable_scene_data *growable_data)
{
	...
	data_out->group_count = growable_data->group_list.item_count;
	...
	data_out->group_list = (obj_group**)growable_data->group_list.items;
