...
typedef struct obj_material
{
	...
	char texture_filename[OBJ_FILENAME_LENGTH]; // map_Ka
	char map_Kd[OBJ_FILENAME_LENGTH];
	char map_Ks[OBJ_FILENAME_LENGTH];
	char map_d[OBJ_FILENAME_LENGTH];
	char bump[OBJ_FILENAME_LENGTH];
	double amb[3];
	double diff[3];
	double spec[3];
	double emit[3];
	double transmit_filter[3];
	double reflect;
	double refract;
	double trans; // alpha
	double shiny;
	double glossy; // not used
